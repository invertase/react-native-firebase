...
3. **Regenerate all native artifacts** when the **mobile** RN/codegen line changes: `yarn codegen:all`, then review the diff — macro/podspec/JSI churn is expected on an RN bump.
...
5. `yarn codegen:verify` must pass (it re-runs codegen and fails on any drift). If it flags drift on an unchanged RN, the mobile toolchain is mis-pinned — fix the `tests/` pins, do not commit the drift.
