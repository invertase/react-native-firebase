...
1. **Bump the mobile app RN (and macOS only when that stack moves).** Update `tests/package.json` (`react-native`, CLI / `@react-native/*`) for mobile + codegen. Update `tests-macos/package.json` (`react-native`, `react-native-macos`) only when the macOS pair advances. Do **not** reintroduce blanket root `resolutions` for `react-native` / `@react-native/codegen` / CLI — each app pins its own line; codegen resolves from **`tests/`**. Never float the toolchain (`latest`, `*`, `^x.y`). Individual packages must not declare their own `react-native` devDependency. See [test app dependency pins](okf-bundle/testing/test-app-dependency-pins.md).
2. `yarn install` and verify pins: from `tests/` and `tests-macos/` report their expected `react-native` versions; `rg 'react-native@npm:' yarn.lock` should show the intended line(s) (plus `react-native-macos` for the macOS app).
3. **Regenerate all native artifacts** when the **mobile** RN/codegen line changes: `yarn codegen:all` (wipe-then-regen on each configured `--outputPath` via [`scripts/codegen-package.mjs`](scripts/codegen-package.mjs) — [NewArch-AD-22](okf-bundle/new-architecture/architecture-decisions.md#newarch-ad-22--codegen-is-wipe-then-regen-on-the-configured-outputpath--accepted)), then review the diff — macro/podspec/JSI churn is expected on an RN bump.
4. **Rebuild native on both platforms** (iOS + Android) and run the e2e suite; generated changes only take effect after a native rebuild. Rebuild/run macOS Jet from `tests-macos/` when that app's native shell or Metro entry changed.
5. `yarn codegen:verify` must pass (it re-runs wipe+regen via `codegen-package.mjs` and fails on any drift). If it flags drift on an unchanged RN, the mobile toolchain is mis-pinned — fix the `tests/` pins, do not commit the drift.
