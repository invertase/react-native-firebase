#!/usr/bin/env bash
# Report whether e2e resources for the current env (or mellifera reservation) are busy.
...
#   bash scripts/e2e/check-e2e-resources.sh --mellifera           # opt in to tests/mellifera.env.json
...
JSON=0
SERVICES=0
# Finding B: initialize from an already-exported parent flag (e.g. release-e2e-resources.sh
# invoking this script as a child) instead of forcing 0 — argv --mellifera below still wins
# and callers should pass it through explicitly rather than relying on inherited env alone.
E2E_MELLIFERA_FLAG="${E2E_MELLIFERA_FLAG:-0}"
for arg in "$@"; do
  case "$arg" in
    --json)
      JSON=1
...
# platform_explicit: true only when platform intent was explicit (--platform, mellifera
# JSON, RNFB_E2E_PLATFORM, or a per-platform port env) — not the ambiguous "global"
