...
E2E_SERIAL_ANDROID_CONSOLE_PORT=5554
...
# Serial yarn tests:android:* must export these so Detox does not FreePortFinder
# 10000–20000 (emulator-16xxx leftovers). Call after dropping slot carry-in.
e2e_pin_serial_android_env() {
  export RNFB_ANDROID_CONSOLE_PORT="$E2E_SERIAL_ANDROID_CONSOLE_PORT"
...
e2e_slot_env_apply() {
  ...
  case "$platform" in
    ...
    macos)
      ;;
    *)
      echo "error: platform must be android|ios|macos (got $platform)" >&2
      return 1
      ;;
  esac
}

# Slot carry-in + process-local binds that babel-plugin-transform-inline-environment-variables
# and host waiters (Detox firebase.test.js, macOS .jetrc.js, start-packager) read.
# RNFB_E2E_DEBUG is intentionally omitted — serial runs may keep verbose debug.
e2e_slot_carry_in_keys() {
  printf '%s\n' \
    RNFB_E2E_SLOT RNFB_E2E_HOST_SLOT \
    ...
    RNFB_E2E_PLATFORM AVD_NAME SIMCTL_CHILD_RCT_METRO_PORT \
    ORG_GRADLE_PROJECT_reactNativeDevServerPort
}

# Serial yarn tests:* must not inherit a prior eval of export-slot-env from the
# agent shell. When no slot is selected, drop carry-in so Metro/Jest/Jet wait
# on :8081 / :8090 / TestingAVD / iPhone 17 / io.invertase.testing, then pin
# Android console 5554 / emulator-5554 (same Detox pin as slotted 5556+2n).
# No-op when RNFB_E2E_SLOT or RNFB_E2E_HOST_SLOT is set (slotted Law).
e2e_sanitize_serial_env() {
  ...
  while IFS= read -r k; do
    [[ -n "$k" ]] || continue
    unset "$k"
  done < <(e2e_slot_carry_in_keys)
  e2e_pin_serial_android_env
}

# Debug APK Metro port is baked at :build (ORG_GRADLE_PROJECT_reactNativeDevServerPort →
# react_native_dev_server_port). Serial :test-cover after a slotted :build still waits on
# host :8081 / reversePorts 8081 while the app asks for slot Metro (e.g. 12007) → Detox
# APP_STATUS currentStatus 5000ms / isReady hang (host prefetch of :8081 still succeeds).
# Call after e2e_sanitize_serial_env (or with slot env loaded). Override path in tests via
# RNFB_ANDROID_APK_METRO_RES.
e2e_assert_android_apk_metro_port() {
  local here repo_root xml expected baked
  here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  repo_root="$(cd "${here}/../../.." && pwd)"
  expected="${RNFB_ANDROID_METRO_PORT:-${RCT_METRO_PORT:-${RNFB_METRO_PORT:-${JET_METRO_PORT:-8081}}}}"
  xml="${RNFB_ANDROID_APK_METRO_RES:-${repo_root}/tests/android/app/build/generated/res/resValues/debug/values/gradleResValues.xml}"
  if [[ ! -f "$xml" ]]; then
    echo "error: missing Android debug Metro resource ${xml} — run yarn tests:android:build in this env first" >&2
    return 1
  fi
  baked="$(sed -n 's/.*<integer name="react_native_dev_server_port">\([0-9][0-9]*\)<\/integer>.*/\1/p' "$xml" | head -1)"
  if [[ -z "$baked" ]]; then
    echo "error: could not read react_native_dev_server_port from ${xml}" >&2
    return 1
  fi
  if [[ "$baked" != "$expected" ]]; then
    echo "error: APK react_native_dev_server_port=${baked} != active Metro ${expected} (${xml}). Re-run yarn tests:android:build in this same serial/slot env (do not :test-cover a slot-baked APK on serial :8081)." >&2
    return 1
  fi
  echo "[rnfb-e2e] android APK Metro port ${baked} matches active ${expected}"
}

# Print `unset KEY` lines for eval into a dirty serial shell (including leftover
# RNFB_E2E_SLOT). Mirror of e2e_slot_env_print. Does not unset RNFB_E2E_DEBUG.
