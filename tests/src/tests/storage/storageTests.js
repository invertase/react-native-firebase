...
function storageTests({ describe, it, firebase, tryCatch }) {
    ...
    it('it should download a file', () =>
      new Promise((resolve, reject) => {
        ...
        firebase.native
          ...
          .ref('/ok.jpeg')
          .downloadFile(
            `${firebase.native.storage.Native.DOCUMENT_DIRECTORY_PATH}/ok.jpeg`
          )
          .then(successCb)
          .catch(failureCb);
      }));
    ...
    it('it should upload a file', () =>
      new Promise((resolve, reject) => {
        const successCb = tryCatch(uploadTaskSnapshot => {
          uploadTaskSnapshot.state.should.eql(
            firebase.native.storage.TaskState.SUCCESS
          );
  ...
  describe('on()', () => {
    it('should listen to upload state', () =>
      new Promise((resolve, reject) => {
        const path = `${
          firebase.native.storage.Native.DOCUMENT_DIRECTORY_PATH
        }/ok.jpeg`;
        const ref = firebase.native.storage().ref('/uploadOk.jpeg');
        const unsubscribe = ref.putFile(path).on(
          firebase.native.storage.TaskEvent.STATE_CHANGED,
          tryCatch(snapshot => {
