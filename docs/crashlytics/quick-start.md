...
Keep in mind that this will not work as expected for non-release environments. On ios, the errors will be missing the dSYM (debug symbols) which are automatically generated by xcode in release archives and so will not appear in
the crashlytics console (you will however see a number of crashes, which crashlytics can't parse into human-understandable errors, under the dSYM tab within the console.) On Android, there will be no reporting of errors at all anywhere in the console.
