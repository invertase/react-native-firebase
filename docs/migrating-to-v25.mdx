...
| `ListOptions.pageToken` required `string`                             | `string \| null` (nullable, matching js-sdk)                                 |
...
**Settings field rename:** modular `RemoteConfigSettings` uses `fetchTimeoutMillis` (firebase-js-sdk), not the older RNFB-style `fetchTimeMillis` on the modular surface.
...
| `PerformanceSettings` RNFB shape                       | `{ dataCollectionEnabled?, instrumentationEnabled? }` (js-sdk) |
...
Modular `getInstallations()` returns a firebase-js-sdk-style `Installations` object exposing only `app`. Use modular helper functions instead of instance methods.
...
- Modular `AppCheck` has no instance methods (matching js-sdk); use free functions.
...
Version 25 aligns `@react-native-firebase/auth` TypeScript types with the firebase-js-sdk modular API. Runtime behaviour is largely unchanged, but TypeScript consumers should review the following breaking changes.
...
- Runtime behaviour should remain compatible for most flows.
