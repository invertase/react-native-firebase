...
For maintainers and coding agents: the living triage matrix is `compare_types_triage.md` at the repository root. Run `yarn compare:types auth` after public API edits and update `.github/scripts/compare-types/configs/auth.ts` when differences are intentional.
...
Provider credential factories return firebase-js-sdk-shaped credential **classes** (`OAuthCredential`, not internal type aliases) with `toJSON()` and `static fromJSON()` where applicable. RN Firebase credentials retain internal `token` / `secret` bridge fields required by the native modules.
...
3. Consult `compare_types_triage.md` for platform context (#8 PhoneAuthProvider MF overloads are native-only extensions; MFA on Other is not missing work).
