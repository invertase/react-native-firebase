---
title: subcollection
description: 'Firestore pipelines subcollection() source helper: target a subcollection, typically paired with aggregate() and toScalarExpression().'
...
snapshot.results.forEach(row => {
  console.log(row.data().name, row.data().reviewSummary);
});
```
