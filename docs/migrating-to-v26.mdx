...
**Modular-only in v26:** `@react-native-firebase/ml`, `@react-native-firebase/in-app-messaging`, `@react-native-firebase/installations`, and `@react-native-firebase/messaging`. Other packages may still expose the namespaced API with deprecation warnings; modular imports remain the supported path.
