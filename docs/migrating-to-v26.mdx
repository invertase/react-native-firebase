...
**Modular-only in v26 so far:** `@react-native-firebase/ml`, `@react-native-firebase/in-app-messaging`. Other packages may still expose the namespaced API with deprecation warnings; modular imports remain the supported path.
