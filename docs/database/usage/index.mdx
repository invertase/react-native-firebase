...
description: 'Realtime Database: ref(), one-time reads with get(), realtime listeners (onValue, onChildAdded), queries (orderBy, limit), writes (set, update, push, remove), runTransaction(), security rules, and connecting to secondary databases.'
...
- Install Database: `npm install --save @react-native-firebase/database` or `yarn add @react-native-firebase/database`, then rebuild native apps (`cd ios && pod install` on iOS).
- Enable Realtime Database in the [Firebase Console](https://console.firebase.google.com/) for your project and deploy security rules appropriate for your app.

Example Expo Router screen: https://github.com/invertase/react-native-firebase/blob/main/test-expo/app/database.tsx
...
const reference = ref(db, '/users/123');
console.log('Secondary-region reference path:', reference.toString());
```
...
const db = getDatabase();
const unsubscribe = onValue(ref(db, '/users/123'), snapshot => {
  console.log('User data: ', snapshot.val());
});

// Call unsubscribe() when the listener is no longer needed.
console.log('Listening; unsubscribe is a function:', typeof unsubscribe);
```
...
async function readScoresOrderedByValue() {
  const db = getDatabase();
  const snapshot = await get(query(ref(db, 'scores'), orderByValue()));
...
async function readFirstTenUsers() {
  const db = getDatabase();
  const snapshot = await get(query(ref(db, 'users'), limitToFirst(10)));
  console.log('First ten users:', snapshot.val());
}
```
...
async function readUsersAgedTwentyOneOrOlder() {
  const db = getDatabase();
  const snapshot = await get(query(ref(db, 'users'), orderByChild('age'), startAt(21)));
  console.log('Users aged 21+:', snapshot.val());
}
```
...
async function removeUserBySettingNull() {
  await set(ref(getDatabase(), '/users/123'), null);
  console.log('User removed by setting null.');
}
```
...
console.log('Secondary database root:', rootRef.toString());
```
...
async function connectSecondaryDatabase() {
  ...
  const rootRef = ref(secondaryDb);
  console.log('Secondary app database root:', rootRef.toString());
  return secondaryDb;
}
```
