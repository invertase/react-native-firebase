...
description: 'Android-only carrier-level phone verification (no SMS): getVerificationSupportInfo() per-SIM checks, getVerifiedPhoneNumber() consent and JWT result, the Digital Credential API flow, PnvErrorCode handling, and enableTestSession() test tokens.'
next: /vertexai/usage
