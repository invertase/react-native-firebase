...
description: 'Migrate from react-native-firebase v5 to v6: install the scoped @react-native-firebase/app plus per-service packages, with breaking changes for AdMob, Analytics, Crashlytics, Firestore, Dynamic Links, Functions, In-App Messaging, Instance ID, Messaging, Performance Monitoring, Realtime Database, Remote Config, Storage and ML.'
