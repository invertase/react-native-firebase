...
- [BREAKING] All Remote Config values can now be accessed synchronously in JS, see `getValue(key: string): Value` & `getAll(): ConfigValues` below
...
- [NEW] Added the following properties to `firebase.remoteConfig()`; `lastFetchTime`, `lastFetchStatus` (typed as `FetchStatus`) & `isDeveloperModeEnabled`
...
- [NEW] All previous `get*` methods have been removed and replaced with 2 synchronous methods:
  - `getValue(key: string): Value` - returns a single configuration value `{ value, source }`
