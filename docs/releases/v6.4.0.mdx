...
- the `requestPermission` & `hasPermission` APIs now returns the current authorization status as an enum, rather than a boolean value.
  - see [`AuthorizationStatus`](https://reference.rnfirebase.io/_react-native-firebase/messaging/index/AuthorizationStatus.html) reference documentation for available enum members
