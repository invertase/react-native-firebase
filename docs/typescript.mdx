---
title: TypeScript
description: 'Modular API import patterns for TypeScript, where the generated type definitions live, and the compare:types process used for Auth.'
next: /platforms
previous: /migrating-to-v27
---

## Prerequisites

- A React Native or Expo app with TypeScript enabled. Official setup: [React Native TypeScript documentation](https://reactnative.dev/docs/typescript).
...
- The product package you import (this page uses `@react-native-firebase/auth` as the example): `npm install --save @react-native-firebase/auth` or `yarn add @react-native-firebase/auth`, then rebuild native apps if you just added the package.
...
Import modular helpers and types from the package root. This example is a complete React component that listens for auth state:
