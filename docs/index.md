...
For iOS only, since `firebase-ios-sdk` requires `use_frameworks` then you want to install the [`expo-build-properties`](https://www.npmjs.com/package/expo-build-properties) package by running `npx expo install expo-build-properties` and then configure [`expo-build-properties`](https://docs.expo.dev/versions/latest/sdk/build-properties/#pluginconfigtypeios) in `app.json` to use `"useFrameworks": "static"` for iOS builds. See the example configuration below.
