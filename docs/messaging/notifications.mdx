---
title: Notifications
description: 'Display and handle FCM notifications sent via the Firebase console, Admin SDKs or REST API: getInitialNotification(), onNotificationOpenedApp(), and advanced local notifications with Notifee.'
...
- On iOS, complete [iOS Messaging Setup](/messaging/usage/ios-setup) and request notification permission (prefer [react-native-permissions](https://github.com/zoontek/react-native-permissions) or [expo-notifications](https://docs.expo.dev/versions/latest/sdk/notifications/); Messaging's `requestPermission` is deprecated — see [issue #6283](https://github.com/invertase/react-native-firebase/issues/6283)).
- On Android API 33+, request `POST_NOTIFICATIONS` before expecting notification UI.
- Register `setBackgroundMessageHandler` in the entry file before `AppRegistry.registerComponent` if you handle background/quit messages (see [Cloud Messaging usage](/messaging/usage#background--quit-state-messages)).
...
If an incoming message with this property exists, and the app is not visible (quit or in the background),
...
a system notification when the app is in the background or quit. In the foreground, no system notification is shown; the
payload is delivered to `onMessage` instead. Messages with both a `notification` and `data` property follow the same split:
foreground uses `onMessage` only (no system notification); background and quit show a system notification and call
`setBackgroundMessageHandler` (not `onMessage`). Display and `onMessage` do not fire together. See the
[message-handler table](/messaging/usage#message-handlers) on Usage.
...
```jsx
// codeblock-ignore: @react-navigation/native and @react-navigation/stack are not workspace dependencies; types unavailable to docs:tsc:check
...
const linking = {
  ...
  subscribe(listener) {
    ...
    const unsubscribe = onNotificationOpenedApp(messaging, remoteMessage => {
      const deepLinkUrl = buildDeepLinkFromNotificationData(remoteMessage.data);
      if (typeof deepLinkUrl === 'string') {
...
A Messaging-only version without React Navigation:
...
function App() {
  useEffect(() => {
    ...
    getInitialNotification(messaging).then(remoteMessage => {
      if (remoteMessage) {
        console.log('Opened from quit state via notification:', remoteMessage.notification);
      }
    });

    const unsubscribe = onNotificationOpenedApp(messaging, remoteMessage => {
      console.log('Opened from background via notification:', remoteMessage.notification);
    });
  ...
  return (
    <View>
      <Text>Notification interaction listeners registered</Text>
    </View>
  );
}
...
To send messages to a device, you need the FCM token for it, which you can get using `getToken(messaging)`.
