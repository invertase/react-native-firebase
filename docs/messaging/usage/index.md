...
> `SendMessage` and it's associated listeners, only fire from a custom `XMPP` server. Read the documentation on how to [Messaging with XMPP](/messaging/usage/messaging-with-xmpp).
