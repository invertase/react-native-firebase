...
description: 'Deprecated iOS notification permission APIs requestPermission()/hasPermission(): AuthorizationStatus values, provisional authorization, and why to prefer react-native-permissions or expo-notifications instead.'
...
- Native Firebase config and [iOS Messaging Setup](/messaging/usage/ios-setup) completed (Push Notifications, Background Modes, APNs). From v26, New Architecture is required for packages with a native bridge.
- Prefer [react-native-permissions](https://github.com/zoontek/react-native-permissions) or [expo-notifications](https://docs.expo.dev/versions/latest/sdk/notifications/) for notification permission requests. The Messaging permission APIs on this page are deprecated (see [issue #6283](https://github.com/invertase/react-native-firebase/issues/6283)).
...
Notifications cannot be shown to users if the user has not "granted" your application permission. The overall notification permission of a single application can be either be "not determined", "granted" or "declined". Upon installing an application, the default status is "not determined".
...
Permission must be requested from your users in order to display remote notifications from FCM, via the
...
console.log('hasPermission:', status, status === AuthorizationStatus.AUTHORIZED);
```
...
2. Handle interaction when the app is in a background state. Register the open-settings handler in the entry file **before** `AppRegistry.registerComponent` (same placement rule as `setBackgroundMessageHandler`):
...
setOpenSettingsForNotificationsHandler(messaging, async () => {
  // Persist a flag your root component reads on mount (AsyncStorage, MMKV, etc.).
  console.log('User opened app notification settings from iOS Settings');
});
...
3. Handle interaction when the app is in a quit state with `getDidOpenSettingsForNotification`:
...
function App() {
  useEffect(() => {
    const messaging = getMessaging();
    getDidOpenSettingsForNotification(messaging).then(didOpenSettingsForNotification => {
      if (didOpenSettingsForNotification) {
        console.log('Opened from iOS notification settings (quit state)');
