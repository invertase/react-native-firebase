...
- Be careful if you are using a VPN, on an Android emulator it might completely mess up the network adapter causing network calls to never return, not only on firebase but on the entire emulated phone, other firebase modules might also become unresponsive behind VPNs.
