...
Community generators exist for these JSON files; they are optional — see [Optional community helpers](#optional-community-helpers) at the end of this page.
...
The [default Hosting path](#choose-a-hosting-path) above is the simplest integration for most apps. If you need a **custom
domain**, a branded continue page, or help generating association files and native snippets, these community-maintained
projects can reduce setup work. They are **not** official React Native Firebase or Firebase products — evaluate them for your
own project.

## No existing website — branded custom domain

[email-link-host](https://github.com/rutvik24/email-link-host) is a static host purpose-built for email-link auth: it
generates `assetlinks.json` and `apple-app-site-association` from environment variables, serves a branded continue page, and
can deploy to Firebase Hosting, Cloudflare Pages, Vercel, Netlify, Amplify, or Docker.
...
## Existing website — generate association files

[App Universal Links Helper](https://rutvik24.github.io/app-universal-links-helper/) (or [run locally](https://github.com/rutvik24/app-universal-links-helper))
generates `assetlinks.json`, `apple-app-site-association`, an AndroidManifest `<intent-filter>` snippet, and optional
`adb` / `curl` / `swcutil` verification commands from your hosts, package name, fingerprints, and path rules (include
`/__/auth/links` for Auth links). It does **not** upload files — publish the JSON under `/.well-known/` on your domain.

## End-to-end sample app

[firebase-email-link-host-demo](https://github.com/rutvik24/firebase-email-link-host-demo) wires React Native modular Auth +
App Links + Universal Links + AsyncStorage. Useful starting points: `src/services/firebaseAuth.ts`, `useEmailLinkAuth.ts`,
and the platform manifest / entitlements files.
