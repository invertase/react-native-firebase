...
description: 'Passwordless email-link sign-in: Android App Links/iOS Universal Links setup, sendSignInLinkToEmail(), isSignInWithEmailLink(), signInWithEmailLink(), linking/re-authenticating, and email enumeration protection.'
next: /auth/oidc-auth
...
- **Email/Password** enabled, then **Email link (passwordless sign-in)** enabled under [Authentication → Sign-in method](https://console.firebase.google.com/project/_/authentication/providers).
- Continue-URL host listed under Authentication → Settings → **Authorized domains**.
- Android: app package name plus **SHA-1 and SHA-256** fingerprints registered in Firebase project settings (needed for App Links on the default Hosting domain).
- iOS: bundle ID matches Xcode / Associated Domains.
- Native App Links / Universal Links claiming the **Hosting link domain** (see Native app setup). Expo needs a [development build](https://docs.expo.dev/develop/development-builds/introduction/) (Expo Go cannot claim `applinks:`).
...
Firebase Auth does not use [Firebase Dynamic Links](https://firebase.google.com/support/dynamic-links-faq) for this
...
official Android and Apple guides. Custom domains and branded landing pages are covered later.
...
This is a **Node Admin SDK** script (not React Native). Install `firebase-admin` in a Node environment; it is not a React Native Firebase workspace dependency, so the fence is excluded from `docs:tsc:check`:
...
File: `android/app/src/main/AndroidManifest.xml` (inside the activity that should receive the link):
...
Persist the email locally **before or immediately after** sending — completing sign-in requires
the same address, and you must not put it in the continue URL query string. Production apps typically use
`@react-native-async-storage/async-storage` (or another durable store). The example below uses an in-memory map so the
snippet type-checks without that dependency; swap in AsyncStorage in your app.
...
async function completeFromUrl(url) {
  ...
  const email = memoryStore[EMAIL_KEY];
  if (!email) {
...
async function linkOrReauthenticateWithEmailLink(email, url, mode) {
...
await linkOrReauthenticateWithEmailLink(
  'jane.doe@example.com',
  'https://YOUR_PROJECT_ID.firebaseapp.com/__/auth/links?link=...',
  'link',
);
```
...
| `auth/invalid-action-code` | Link expired or already used; send another link. |
