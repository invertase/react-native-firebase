...
be configured, call the documented fallback from `post_integrate` (use
`post_install` only if your CocoaPods version has no `post_integrate` hook):
...
- **Archive fails with `Undefined symbols ... _OBJC_CLASS_$_FIRApp`:** RNFB
  ...
  (or `post_install` on CocoaPods without that hook) as a fallback if your
  own native code also calls Firebase APIs directly.
