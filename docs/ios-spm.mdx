...
description: 'Swift Package Manager and CocoaPods support for Firebase iOS SDK dependencies: requirements, dynamic linkage, and Expo configuration.'
next: /faqs-and-tips
previous: /platforms
---
...
- `@react-native-firebase/app` installed (and any other `@react-native-firebase/*` packages you use). Complete [Getting Started](/) so `GoogleService-Info.plist` / Expo `googleServicesFile` and `FirebaseApp.configure()` (React Native CLI) are in place.
- React Native **0.75 or newer** for the SPM default. Older React Native versions use CocoaPods because their pod tooling does not provide `spm_dependency`.
- Xcode `>= 26.2` when using firebase-ios-sdk `12.12.0` or newer (this release pins firebase-ios-sdk `12.19.0`).
- Expo projects: use config plugins (`expo-build-properties`, `@react-native-firebase/app`); do not hand-edit a generated `ios/Podfile` if Expo regenerates it on prebuild.
...
SPM requires dynamic framework linkage. Edit `ios/Podfile` **before** `pod install`:

```ruby
# File: ios/Podfile. Inside your app target, before use_react_native!. Required for SPM mode. Run pod install after editing.
...
Write this into `app.json` / `app.config.js` **before** `npx expo prebuild`:

```json
// File: app.json (or app.config.js). Apply before npx expo prebuild. Sets iOS useFrameworks for SPM (dynamic).
{
  "expo": {
    "plugins": [
      [
...
and configure the linkage your project requires. Apply **before** the next prebuild:

```json
// File: app.json (or app.config.js). CocoaPods opt-out via disableSPM. Apply before npx expo prebuild.
{
  "expo": {
    "plugins": [
      [
...
linkage in your `ios/Podfile`, then install pods:

```ruby
# File: ios/Podfile. Inside your app target, before use_react_native!. Step 1 of 2 for SPM (React Native CLI).
use_frameworks! :linkage => :dynamic
```

```bash
# Project root / ios/. Step 2 of 2 for SPM (after Podfile use_frameworks! :linkage => :dynamic).
cd ios
pod install
```
...
`post_install` block in `ios/Podfile`, after `react_native_post_install`, then
re-run `pod install`:

```ruby
# File: ios/Podfile. Fallback only when pod install warns SPM build settings were not applied. Place after react_native_post_install, then pod install.
...
be configured, call the documented fallback from `post_integrate` in
`ios/Podfile`, then re-run `pod install`:

```ruby
# File: ios/Podfile. Fallback only when pod install warns SPM embed phase was not configured. Then pod install.
...
To opt out of SPM, add this before any target block in your `ios/Podfile`, then
run `pod install`:

```ruby
# File: ios/Podfile. Before any target block. Step 1 of CocoaPods opt-out; then set linkage and pod install.
...
native consumer. Set this in `ios/Podfile` before any target, then `pod install`:

```ruby
# File: ios/Podfile. Before any target block. Required to share one FirebaseCore with other CocoaPods Firebase consumers.
$RNFirebaseDisableSPM = true
```
...
- **Release archive fails with an `xcframework-ios.signature` "couldn't be
  ...
  ```bash
  # Xcode app target → Build Phases → New Run Script. Only if archive fails with xcframework-ios.signature copy conflict and RNFB could not apply its automatic workaround.
...
Release configuration. Add this inside your existing `post_install` block in
`ios/Podfile` (after reproducing the archive-only failure), then re-run
`pod install`:

```ruby
# File: ios/Podfile. Inside post_install. tvOS hybrid-linkage TestFlight workaround only; increases binary size.
