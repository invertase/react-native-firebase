...
SPM products are dynamic, and static linkage can make multiple RNFB pods link
...
- `SWIFT_ENABLE_EXPLICIT_MODULES = 'NO'` on both the app project and the Pods
  project, so Xcode 26's explicit Swift modules don't fail to resolve
  Firebase's internal-only SPM targets (`FirebaseCoreInternal`,
  `FirebaseSharedSwift`). This does not disable SPM -- it makes Swift use
  implicit module discovery across the app, CocoaPods, and SPM build boundary.
...
- **Duplicate Firebase symbols while linking:** Do not combine RNFB SPM mode
  with static linkage. Use dynamic linkage or set
  `$RNFirebaseDisableSPM = true`.
- **`FirebaseCoreInternal` or `FirebaseSharedSwift` cannot be resolved, or a
  Release/Archive build crashes at launch with a missing Firebase symbol:**
  RNFB applies `-ObjC` and `SWIFT_ENABLE_EXPLICIT_MODULES = 'NO'`
  automatically; call `rnfirebase_apply_spm_build_settings(installer)` as a
  fallback only if `pod install` warns it could not do so.
