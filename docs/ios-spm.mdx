...
### Xcode 26 module-resolution errors

If Xcode reports that a Firebase module such as `FirebaseCoreInternal` or
`FirebaseSharedSwift` cannot be resolved, disable explicit Swift modules for
both the app project and the Pods project. Add the following inside your
...
This setting does not disable SPM. It makes Swift use implicit module discovery
...
- **Duplicate Firebase symbols while linking:** Do not combine RNFB SPM mode
  ...
  `$RNFirebaseDisableSPM = true`.
- **`FirebaseCoreInternal` or `FirebaseSharedSwift` cannot be resolved:** Apply
  `SWIFT_ENABLE_EXPLICIT_MODULES = 'NO'` to both the app and Pods projects as
  shown above.
