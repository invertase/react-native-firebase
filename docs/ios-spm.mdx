...
0.75+ default). This matters for generated or templated Podfiles that always
emit the variable, e.g. `$RNFirebaseDisableSPM = false`: that still uses SPM,
it does not fall back to CocoaPods.
