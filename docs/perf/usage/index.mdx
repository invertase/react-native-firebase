...
description: 'Performance Monitoring: trace()/startScreenTrace() for custom and screen traces, httpMetric() for HTTP request latency, and the perf_auto_collection_enabled firebase.json flag with setPerformanceCollectionEnabled().'
