...
/**
 * Prepends appName and url arg to all native method calls
 * @param appName
 * @param url
 * @param NativeModule
 */
export function nativeWithAppAndURL(appName: string, url: string, NativeModule: Objectg) {
  const native = {};
  const methods = Object.keys(NativeModule);
  ...
  for (let i = 0, len = methods.length; i < len; i++) {
    const method = methods[i];
    native[method] = (...args) => {
      return NativeModule[method](...[appName, Boolean(url) ? url : null, ...args]);
    };
  }
