...
async function waitForMetroMacosBundle(metroPort = 8081, timeoutMs = 600000) {
  ...
  const statusTimeoutMs = 120000;
  let statusOk = false;

  while (Date.now() - started < statusTimeoutMs) {
    try {
  ...
  if (!statusOk) {
    throw new Error(
      `Metro not responding with packager-status:running on 127.0.0.1:${metroPort} after ${statusTimeoutMs}ms`,
    );
  }
