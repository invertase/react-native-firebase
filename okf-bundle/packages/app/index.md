...
`test-expo/` is a user-facing Expo example (Expo Router, one route per included inventory package) **and** the documented-path iOS **link** closer. It is not Detox e2e. Canonical closer: `yarn test-expo:ios:link` from repo root (build-only; typecheck and JS lint rows in [agent command policy](../../testing/agent-command-policy.md)). Native `RNFB*` discovery and named App+Messaging signatures: [iOS SPM native integration § Expo precompiled module linkage repair](../../ios-spm-native-imports.md#expo-precompiled-module-linkage-repair). App-target FirebaseCore linking and CocoaPods hook order: [iOS SPM native integration](../../ios-spm-native-imports.md#app-target-firebasecore-link-package-dependency-alone-is-not-enough).
...
* [`test-expo/`](../../../test-expo/) — workspace Expo example and documented-path iOS link closer (not Detox; closer is build-only)
