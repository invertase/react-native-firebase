...
| Check | Requirement |
|-------|-------------|
| **Canonical location** | Each topic has one owning doc; others link to it ([change authoring](testing/change-authoring-workflow.md) for workflow/gates/frozen tree, [running e2e](testing/running-e2e.md) for **all** e2e commands — never invoke Jet/Detox/Metro directly; follow [agent rule](testing/running-e2e.md#agent-rule-read-first), [platform coverage gate](testing/running-e2e.md#platform-coverage-gate-blocking), [iteration vocabulary](testing/iteration-vocabulary.md) for term ids only, this file for doc/commit policy, etc.) |
