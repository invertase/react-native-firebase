...
- **`yarn docs:tsc:check` console head:** the harness always exits **0**; when fences fail it prints only the first 10 failures. Confirm one docs area from gitignored `.docs-snippets-tsc/`, not from that head (a clean area can be absent from the printed slice).
