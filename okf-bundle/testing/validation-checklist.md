...
- **`yarn docs:tsc:check` console head:** the harness always exits **0**; when fences fail it prints only the first 10 failures. Confirm one docs area from gitignored `.docs-snippets-tsc/`, not from that head (a clean area can be absent from the printed slice). Do not confirm with `yarn tsc --project tsconfig.docs-snippets.generated.json <files>` (TS5042: project cannot mix with source files) or a full-project `yarn tsc -p` (the `tsc` CLI suppresses cross-file diagnostics). Area confirmation needs the TypeScript compiler API against `.docs-snippets-tsc/`, the same approach as `scripts/docs-snippets-tsc.mjs`.
