...
Unslotted `:test-cover` on a host is **serialized** because Android, iOS, and macOS share Jet (`:8090`), Firebase emulators (fixed ports), devices, coverage output paths, and — after the `tests-macos/` split — **cannot share one Metro**: mobile Jet uses `yarn tests:packager:*` (`tests/` on `:8081`), macOS Jet uses `yarn tests:macos:packager:*` (`tests-macos/` on the same serial `:8081`). OKF **serial default** still reflects that ([running e2e § Rules #6](running-e2e.md#rules)).
...
| `run-worktree-parallel.sh` | **Two Metros** when macos is in the wave (`tests/` + `tests-macos/`) + per-platform Jet/emulators; three `:test-cover` parallel |
