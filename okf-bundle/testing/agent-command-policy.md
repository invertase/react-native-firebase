...
| Slotted parallel env / packager / `:test-cover`                 | [running e2e § slot lifecycle](running-e2e.md#slot-lifecycle) — `export-slot-env.sh`, `check-e2e-resources.sh` / `release-e2e-resources.sh` (with env loaded = slot-scoped; unscoped wipe includes leftover slotted ports + `--devices` for `TestingAVD-N` / iOS slot sims), `start-emulator-slotted.sh` (aborts if suite ports busy), `run-slotted-packager.sh`, Apple `yarn tests:<platform>:pod:install` then `yarn tests:<platform>:build` (compile only; skip `pod:install` for Android), `run-slotted-test-cover.sh` (or yarn `tests:e2e:export-slot-env` / `tests:e2e:slotted-*`). macOS packager is `yarn tests:macos:packager:*` (`tests-macos/`); mobile is `yarn tests:packager:*` (`tests/`) | ad-hoc port math; `.r4-*` scratch launchers; hardcoding `io.invertase.testing` for concurrent macOS; unscoped release while another slot is running; using the mobile packager for macOS Jet (or vice versa); treating firebase “port taken” as retryable flake |
...
| Ad-hoc Metro / emulator start                                                                                                                                                    | Use `yarn tests:packager:jet` (iOS/Android) or `yarn tests:macos:packager:jet` (macOS), `yarn tests:emulator:start`. Does **not** forbid allowlisted `run-slotted-packager.sh` / `start-emulator-slotted.sh` ([registry](#canonical-registry)) |
...
- First-time / Gemfile change: `bundle install` at repo root (path via committed `.bundle/config`). CI uses `BUNDLE_FROZEN=true` against the committed lockfile (+ checksums). Do not `gem update cocoapods xcodeproj`.
