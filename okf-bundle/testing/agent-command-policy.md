...
description: Canonical allowlist for agent shell commands — install, prepare, validation, e2e, Expo example typecheck and documented-path iOS link, RN CLI prebuilt RNCore iOS build, and workflow-friction logging. Supersedes improvised diagnostics.
...
Single source for **which shell commands agents may run** in this repo. E2e `yarn tests:*` detail lives in [running e2e](running-e2e.md) ([agent rule](running-e2e.md#agent-rule-read-first)). `test-expo/` is a user-facing Expo example **and** the documented-path iOS **link** closer (not Detox; the closer is **build-only**). `test-rn-bare/` is the RN CLI prebuilt RNCore iOS **build** fixture (not Detox, not an example app). Each command is only its registry row below. Ad-hoc `pod` / `xcodebuild` stay **never-use** (with or without those rows).
...
| Android Java format / lint                                      | `yarn lint:android` (Java only; not Kotlin)                                                                                                                                                                                                                                                | `yarn google-java-format`, bare `google-java-format`, `google-java-format -i`, `npx google-java-format`, using this script on `.kt`                           |
| Android Kotlin format                                           | repo-root `./gradlew ktlintFormat` with `-PinternalKtlintGitFilter=` (newline-separated repo-root-relative `packages/*/android/**/*.kt` and, if touched, `tests/android/**/*.kt`)                                                                                                          | `ktlint` / `npx ktlint`; yarn format scripts; `cd tests/android && ./gradlew …`; other Gradle ktlint tasks; `-PrnfbKtlintGitFilter` on the **root** wrapper |
...
| Workflow friction — read the inventory at item pickup; record friction met during the work | `yarn frog list` (current friction inventory); `yarn frog log` (interactive — record the friction just encountered)                                                                                              | Direct/global `frog`; `npx` / `yarn dlx` (and other package-runner downloads); reading, grepping, or hand-editing storage; ad-hoc friction records; inventing other `frog` subcommands or flags |
...
| `yarn tsc:compile`, `yarn tsc:compile:test-expo`, Jest, `compare:types` | May read transpiled output or assume `dist/` is current                                        |
...
| `yarn google-java-format`, bare `google-java-format`, `npx google-java-format`, `google-java-format -i`                                                                          | Invented format entrypoints — **only** `yarn lint:android` for **Java**             |
| `ktlint`, `npx ktlint`, invented `yarn` ktlint scripts, `cd tests/android && ./gradlew ktlintFormat`                                                                              | Invented Kotlin format — **only** repo-root `./gradlew ktlintFormat`                |
...
| Ad-hoc `./gradlew …` outside allowlisted yarn scripts (`tests:android:unit`, `tests:android:build`, `tests:android:post-e2e-coverage`, `tests:android:test:jacoco-report`, etc.) and outside repo-root `./gradlew ktlintFormat` | Wrong task / cwd / report path; invents CI that does not match Codecov              |
...
| Ad-hoc `expo prebuild`, `xcodebuild`, `cd test-expo && tsc`, or `cd test-expo && …` as a test-expo link, typecheck, or launch gate                                                 | **Only** `yarn test-expo:ios:link` / `yarn tsc:compile:test-expo` from repo root — not Detox / `yarn tests:*` / `yarn test-rn-bare:ios:build` |
...
- **Canonical:** `yarn lint:android` (repo root) — wraps `google-java-format --set-exit-if-changed --replace` on **Java** under `packages/*/android/src` and fails if the tree would change. It does **not** format Kotlin.

<a id="android-kotlin-format"></a>

### Android Kotlin format

- **Canonical:** repo-root `./gradlew ktlintFormat` (the root wrapper forwards into `tests/android`). Do **not** invent a yarn wrapper, a `ktlint` CLI, or `cd tests/android && ./gradlew …`.
- Pass `-PinternalKtlintGitFilter=` with newline-separated **repo-root-relative** paths (`packages/<pkg>/android/**/*.kt`; `tests/android/**/*.kt` when those files are in the diff). The root wrapper remaps that property; do not pass `-PrnfbKtlintGitFilter` to **root** `./gradlew`. Omit or empty filter → **no** `.kt` files formatted.
- Other Gradle tasks stay yarn-only (`tests:android:unit` / `:build` / Jacoco rows). `ktlintFormat` is the Gradle exception, not a license to invent Gradle.
...
- `yarn lint:js` is `eslint packages/* test-expo --max-warnings=0`. That already covers `packages/app/__tests__/` and `test-expo/`. Do not invent a second lint entrypoint for `test-expo/`. `scripts/version.js` is not in it.
...
### expo-router autolink exclude (`@expo/ui`)

<a id="expo-router-autolink-exclude-expo-ui"></a>

`expo-router` depends on `@expo/ui`. Yarn can resolve a `@expo/ui` that does not compile against the fixture's `expo-modules-core` (`ContentOriginRegistry` / `ShadowNodeProxy`). `test-expo` does not use Expo UI. Keep `expo.autolinking.exclude: ["@expo/ui"]` in `test-expo/package.json`. Do not drop it as unused config. Dropping it makes `yarn test-expo:ios:link` fail at compile (exit 65).
...
Expo example + documented-path iOS link (not Detox, build-only): yarn test-expo:ios:link ONLY — never ad-hoc expo prebuild / xcodebuild / cd test-expo; never yarn test-rn-bare:ios:build as that closer. Typecheck: yarn tsc:compile:test-expo ONLY — never cd test-expo && tsc. JS lint is yarn lint:js (already includes test-expo/). Keep test-expo/package.json expo.autolinking.exclude ["@expo/ui"] — #expo-router-autolink-exclude-expo-ui.
...
Never invent format/install: yarn google-java-format, bare/npx google-java-format, npm install, yarn install in tests/ alone — use root yarn first; Java format = yarn lint:android ONLY (not Kotlin). Kotlin format = repo-root ./gradlew ktlintFormat ONLY with -PinternalKtlintGitFilter= (repo-root-relative packages/*/android/**/*.kt and, if touched, tests/android/**/*.kt); never ktlint CLI / npx ktlint / invented yarn ktlint. See #android-kotlin-format.
Never invent Android Gradle: ad-hoc ./gradlew outside yarn tests:android:unit / :build / :post-e2e-coverage / :test:jacoco-report and outside repo-root ./gradlew ktlintFormat; bare detox/jet/metro.
