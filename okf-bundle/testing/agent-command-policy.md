...
| Docs snippet type-check (non-blocking)                          | `yarn docs:tsc:check` — advisory only, not wired into `yarn lint` / `yarn lint:markdown` / `yarn lint:all` / CI ([validation checklist § lint and formatting](validation-checklist.md#lint-and-formatting))                                                                                     | ad-hoc `tsc` on doc fences / hand-copied snippets                                                                                                             |
...
Docs snippet type-check (advisory): yarn docs:tsc:check — extracts docs/**/*.mdx js/jsx fences, compiles against tsconfig.consumer.json types. Non-blocking; not part of yarn lint / lint:markdown / lint:all / CI. Never invent ad-hoc tsc on doc fences instead.
