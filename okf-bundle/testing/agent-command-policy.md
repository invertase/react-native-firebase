...
| Slotted parallel env / packager / `:test-cover`                 | [running e2e § slot lifecycle](running-e2e.md#slot-lifecycle) — `export-slot-env.sh`, `check-e2e-resources.sh` / `release-e2e-resources.sh` (with env loaded = slot-scoped), `start-emulator-slotted.sh`, `run-slotted-packager.sh`, `yarn tests:<platform>:build`, `run-slotted-test-cover.sh` (or yarn `tests:e2e:export-slot-env` / `tests:e2e:slotted-*`). macOS packager is `yarn tests:macos:packager:*` (`tests-macos/`); mobile is `yarn tests:packager:*` (`tests/`) | ad-hoc port math; `.r4-*` scratch launchers; hardcoding `io.invertase.testing` for concurrent macOS; unscoped release while another slot is running; using the mobile packager for macOS Jet (or vice versa) |
