...
| Slotted parallel env / packager / `:test-cover`                 | [running e2e § parallel topology](running-e2e.md#parallel-e2e-topology) — `bash scripts/e2e/export-slot-env.sh`, `run-slotted-packager.sh`, `run-slotted-test-cover.sh`, `start-emulator-slotted.sh` (or yarn `tests:e2e:export-slot-env` / `tests:e2e:slotted-*`). macOS packager is `yarn tests:macos:packager:*` (`tests-macos/`); mobile is `yarn tests:packager:*` (`tests/`) | ad-hoc port math; `.r4-*` scratch launchers; hardcoding `io.invertase.testing` for concurrent macOS; using the mobile packager for macOS Jet (or vice versa) |
