...
- [x] **1.3** Refactor `packages/app-check/lib/web/RNFBAppCheckModule.ts`:
...
- [x] **1.6** Update `.github/scripts/compare-types/configs/app-check.ts` — remove `ReCaptchaEnterpriseProvider` / `ReCaptchaV3Provider` from `missingInRN`; narrow `AppCheckOptions` / `CustomProvider` `differentShape` reasons.
- [x] **1.7** Update `packages/app-check/type-test.ts` — provider classes, `'recaptcha'` options, provider-less init.
