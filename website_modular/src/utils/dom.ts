...
export function getModuleTypes(document: Document): ModuleTypes {
  ...
  elements.forEach(element => {
    types[element.innerHTML] = element.getAttribute('href').replace('.md', '');
  });
...
export function highlightDefinitions(document: Document, types: ModuleTypes) {
  document.querySelectorAll('pre[data-sanitized="true"] > div').forEach(node => {
    ...
    html = html.replace(
      ...
      v => `<span style="color: #d4d0ab;">${v.toLowerCase()}</span>`,
    );

    // Colorize any known types, and provide a link to the page.
    Object.entries(types).forEach(([type, href]) => {
      const regex = new RegExp(`(${type})`, 'g');
      html = html.replace(regex, `<a href="${href}" style="color: #00e0e0">$1</a>`);
    });
