 ...
 * HandleMap lock. Replace is {@link #registerReplacing} ({@code take} then {@code put}).
 */
final class RNFBDatabaseTransactionRegistry {
  ...
  /**
   * Replace = take then unique put. The taken handler is not aborted (Firebase retry). Collision
   * aborts the incoming handler via {@link #put}.
   */
  void registerReplacing(int transactionId, DatabaseAbortable handler)
      throws RNFBHandleCollisionException {
    take(transactionId);
    put(transactionId, handler);
  }
