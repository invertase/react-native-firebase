...
describe('iid()', function () {
  ...
  describe('namespace', function () {
    ...
    it('supports multiple apps', async function () {
      ...
      firebase.app('secondaryFromNative').iid().app.name.should.equal('secondaryFromNative');
      await Utils.sleep(1000);

      const defaultToken = await firebase.iid().getToken();
      await Utils.sleep(1000);

      const secondaryToken = await firebase.app('secondaryFromNative').iid().getToken();
