 ...
 * is `shouldForwardEvent:listenerId:expected:`.
 */
...
/// When `event[@"done"]` is true, identity-takes `expected` and returns YES if it was mapped.
/// Otherwise returns whether `expected` is still the registered holder (chunk may be forwarded).
