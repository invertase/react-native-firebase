...
public class RNFBFunctionsStreamingRegistryTest {
  ...
  public void shouldEmit_whenPresent_isTrue() throws Exception {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    StreamingHolder holder = new StreamingHolder();
    registry.put(4, holder);
    assertTrue(registry.shouldEmit(4, holder));
  }

  @Test
  public void shouldEmit_whenTaken_isFalse() {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    assertFalse(registry.shouldEmit(4, new StreamingHolder()));
  }

  @Test
  public void shouldEmit_whenDifferentHolderMapped_isFalse() throws Exception {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    StreamingHolder mapped = new StreamingHolder();
    registry.put(4, mapped);
    assertFalse(registry.shouldEmit(4, new StreamingHolder()));
    assertSame(mapped, registry.get(4));
  }
  ...
  public void takeAndShouldEmitComplete_whenHolderPresent_emitsAndClears() throws Exception {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    StreamingHolder holder = new StreamingHolder();
    registry.put(5, holder);

    assertTrue(registry.takeAndShouldEmitComplete(5, holder));
  ...
  public void takeAndShouldEmitComplete_whenAlreadyTaken_skipsEmit() {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    assertFalse(registry.takeAndShouldEmitComplete(5, new StreamingHolder()));
  }

  /**
   * Regression: after this stream's holder is replaced at the same listenerId, complete must not
   * take the replacement.
   */
  @Test
  public void takeAndShouldEmitComplete_whenReplaced_leavesReplacement() throws Exception {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    StreamingHolder original = new StreamingHolder();
    StreamingHolder replacement = new StreamingHolder();
    registry.put(5, original);
    assertSame(original, registry.take(5));
    registry.put(5, replacement);

    assertFalse(registry.takeAndShouldEmitComplete(5, original));
    assertSame(replacement, registry.get(5));
  }
  ...
  public void onExecutorFailure_takesHolder() throws Exception {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    StreamingHolder holder = new StreamingHolder();
    registry.put(6, holder);

    registry.onExecutorFailure(6, holder);

    assertNull(registry.get(6));
  }

  @Test
  public void onExecutorFailure_whenReplaced_leavesReplacement() throws Exception {
    RNFBFunctionsStreamingRegistry registry = new RNFBFunctionsStreamingRegistry();
    StreamingHolder original = new StreamingHolder();
    StreamingHolder replacement = new StreamingHolder();
    registry.put(6, original);
    assertSame(original, registry.take(6));
    registry.put(6, replacement);

    registry.onExecutorFailure(6, original);

    assertSame(replacement, registry.get(6));
  }
