...
describe('functions() modular', function () {
  ...
  describe('modular', function () {
    ...
    describe('streaming', function () {
      ...
      it('httpsCallable(functions, name).stream() emits chunks and ends with done', async function () {
        const { getApp } = modular;
        const { getFunctions, httpsCallable, connectFunctionsEmulator } = functionsModular;
        const region = 'us-central1';
        const fnName = 'helloWorldV2';
      ...
      it('httpsCallableFromUrl(functions, url).stream() emits chunks and ends with done', async function () {
        const { getApp } = modular;
        const { getFunctions, httpsCallableFromUrl } = functionsModular;
        let hostname = 'localhost';
        if (Platform.android) {
