 ...
 * NSMutableDictionary-upserted use `putReplacing` (`take` then `put`, last wins). Peek with `get`;
 * remove with `take`; invalidate with `clear`. Unique `put` errors on collision; `putOrDiscard`
...
/// Replace = take then unique put. Preserves prior dictionary upsert (last wins).
