...
class FirebaseMlKitLanguageModule extends FirebaseModule {
  ...
  suggestReplies(messages) {
    if (!isArray(messages)) {
      throw new Error(
        "firebase.mlKitLanguage().suggestReplies(*) 'messages' must be an array value.",
      );
    }
    ...
    for (let i = 0; i < messages.length; i++) {
      try {
        ...
        throw new Error(
          `firebase.mlKitLanguage().suggestReplies(*) 'messages' object at index ${i} threw an error. ${e.message}.`,
        );
      }
    }
