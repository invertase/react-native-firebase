...
@protocol RNFBFirestoreRemovable <NSObject>
- (void)remove;
@end

@interface RNFBFirestoreListenerRegistry ()
@property(nonatomic, strong) RNFBHandleMap *map;
@end

@implementation RNFBFirestoreListenerRegistry
...
- (void)rnfb_removeHandle:(id)handle {
  if (handle && [handle respondsToSelector:@selector(remove)]) {
    [(id<RNFBFirestoreRemovable>)handle remove];
  }
}
