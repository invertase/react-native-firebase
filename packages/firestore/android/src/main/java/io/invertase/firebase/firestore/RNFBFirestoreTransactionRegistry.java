...
/**
 * Transaction handler map. Unique {@link #put}; begin uses {@link #putOrSkip} (atomic {@link
 * RNFBHandleMap#putIfAbsentOrSame}). Callers {@link #take} or {@link #takeAllAndAbort} then {@code
 * abort()} outside the HandleMap lock.
 */
final class RNFBFirestoreTransactionRegistry {
   ...
   * value returns false and leaves the existing mapping. Single HandleMap lock via {@link
  ...
  boolean putOrSkip(int transactionId, ReactNativeFirebaseFirestoreTransactionHandler handler) {
    return map.putIfAbsentOrSame(transactionId, handler);
  }
