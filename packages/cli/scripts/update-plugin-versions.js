...
async function getMavenVersions(namespace, plugin) {
  try {
    ...
    console.error(`Unable to fetch version information for ${namespace}:${plugin}: ${e.message}`);
    return null;
  }
}
...
for (const [namespace, plugins] of Object.entries(oldVersions)) {
  ...
  for (const [plugin, version] of Object.entries(plugins)) {
    ...
    request.then(versions => {
      ...
      if (version) {
        if (isNew)
          console.log(`Plugin ${namespace}:${plugin} version updated from ${version} to ${latest}`);
